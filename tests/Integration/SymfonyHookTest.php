...
final class SymfonyHookTest extends TestCase
{
    ...
    {
        $this->healTo(['limit' => 100]);
        $response = (new \Symfony\Component\HttpClient\NativeHttpClient())
    ...
    public function testRetryPreservesPerRequestOptionsAndRemovesHeaders(): void
    {
        ...
        $client = HttpClient::create(['headers' => ['X-Old' => 'remove'], 'auth_bearer' => 'local-secret']);
    ...
    {
        $response = HttpClient::create()->request('GET', $this->upstream->url.'/orders?limit=500', ['buffer' => false]);
        self::assertSame(400, $response->getStatusCode());
        self::assertStringContainsString('too big', $response->getContent(false));
        self::assertSame([], $this->manifest->heals());
    }
