...
final class GuzzleHookTest extends TestCase
{
    ...
    public function testClientDefaultsCannotOverwriteAHealedRequest(): void
    {
        $mock = new MockHandler([new Response(400), new Response(200)]);
        $this->manifest->setResult(['status' => 'patched', 'healAttemptId' => 'a1', 'healedRequest' => [
            'url' => 'https://api.example/orders?limit=100',
        ...
        self::assertSame(200, $client->post('https://api.example/orders')->getStatusCode());
        $retry = $mock->getLastRequest();
        self::assertSame('limit=100', $retry->getUri()->getQuery());
        self::assertSame('{"limit":100}', (string) $retry->getBody());
        self::assertFalse($retry->hasHeader('X-Old'));
        self::assertFalse($retry->hasHeader('Authorization'));
    }

    public function testSuccessfulRetryStreamsAreNotConsumed(): void
    {
        ...
        self::assertSame(100000, strlen($response->getBody()->getContents()));
    }

    public function testLargeFailuresKeepTheirPrefixAndRemainReadable(): void
    {
        $raw = str_repeat('x', \Mnfst\Wire::RESPONSE_BODY_CAP + 1000);
        ...
        self::assertSame(\Mnfst\Wire::RESPONSE_BODY_CAP + 1, $stream->bytesRead);
        self::assertSame(\Mnfst\Wire::RESPONSE_BODY_CAP, strlen($captured['body']));
        self::assertTrue($captured['truncated']);
        self::assertSame(hash('sha256', $raw), hash('sha256', $response->getBody()->getContents()));
    }
